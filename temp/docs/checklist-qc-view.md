...
## Prefilter Checklist (QC View)
- [ ] PF-QV-01 Prefilter bar is visible and can be collapsed/expanded after apply.
- [ ] PF-QV-02 Search by field is not shown on QC View.
- [ ] PF-QV-03 Owner selector is hidden (current user is implicit).
- [ ] PF-QV-04 Work That options are: Is assigned to me; I have completed; Is assigned to me but is being progressed by the caseworker.
- [ ] PF-QV-05 Completed date range is required only for "I have completed".
- [ ] PF-QV-06 "From date" cannot be in the future; error shown if it is.
- [ ] PF-QV-07 "From date" auto-sets "To date" to min(from + 14 days, today).
- [ ] PF-QV-08 Search is disabled until required fields are set (work that, and from date when required).
- [ ] PF-QV-09 "Clear search" resets prefilter to defaults and clears stored prefilter state.
